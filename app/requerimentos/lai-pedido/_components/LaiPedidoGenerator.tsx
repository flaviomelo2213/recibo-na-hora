...
const generateLaiPDF = (data: LaiPedidoData) => {
  ...
  doc.setFont('Helvetica', 'normal');
  const preambulo = `Eu, ${data.requerenteNome || '[NOME COMPLETO]'}, ` +
  ...
  doc.setFont('Helvetica', 'normal');
  const pedidoText = doc.splitTextToSize(data.pedido || '[Descreva aqui, de forma clara e objetiva, a informação que você busca.]', usableWidth);
  ...
  doc.setFont('Helvetica', 'normal');
  const formaAcesso = `Solicito que as informações sejam fornecidas, preferencialmente, por ${data.formaPreferencial || '[meio eletrônico (e-mail), cópia física, etc]'}.`;
...
export default function LaiPedidoGenerator() {
  ...
  const getPreviewText = () => {
    ...
    return (
        ...
        `2. FORMA DE ACESSO\nSolicito que as informações sejam fornecidas, preferencialmente, por ${data.formaPreferencial || '[meio eletrônico (e-mail), cópia física, etc]'}.\n\n` +
        `Nestes termos,\nPede deferimento.\n\n` +
        ...
        <div className="grid md:grid-cols-2 gap-10">
          ...
          <div className="space-y-6">
            ...
            <div>
              ...
              <div className="space-y-4">
                <Textarea name="pedido" label="Informação Solicitada" value={data.pedido} onChange={handleChange} rows={5} placeholder="Seja específico. Ex: Cópia do contrato nº 123/2023, planilha de gastos com publicidade de 2024, etc." />
            ...
            <div className="text-center pt-4">
              <Button size="lg" onClick={handleGeneratePDF}>Gerar PDF do Requerimento</Button>
            </div>
          </div>
          ...
          <div>
            <div className="sticky top-24">
                <h3 className="text-lg font-semibold text-slate-800 border-b pb-2 mb-4">Pré-visualização</h3>
                ...
                <p className="text-xs text-slate-500 mt-2 text-center">O documento final será um PDF em formato A4.</p>
            </div>
          </div>
