...
export default function LeadModal({ isOpen, onClose, onSuccess }: LeadModalProps) {
  ...
  return (
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-label="Cadastro para liberar download"
    >
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-xl overflow-hidden animate-fade-in max-h-[90vh] flex flex-col">
        ...
        <div className="bg-blue-600 p-6 text-center">
          ...
          <p className="text-blue-100 text-sm mt-1">
            Cadastre-se para liberar o download imediatamente.
          </p>
        </div>
        ...
        <div className="p-6 sm:p-8 overflow-y-auto">
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full bg-green-600 hover:bg-green-700 text-white font-bold py-4 rounded-xl shadow-lg hover:shadow-green-600/30 transition-all transform hover:-translate-y-0.5 flex items-center justify-center gap-2 disabled:opacity-70 disabled:cursor-not-allowed"
            >
              {loading ? (
                'Liberando...'
              ) : (
